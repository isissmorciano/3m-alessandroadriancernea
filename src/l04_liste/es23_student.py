...
# ## Obiettivo
# Definire tre liste parallele: nomi delle città (string), popolazione (int), area in km² (float). Calcolare la densità abitativa 
# (popolazione / area) per ogni città, trovare la città con densità massima, e contare quante città hanno densità >1000 abitanti/km².
...
if len(nomi_citta) == len(popolazione) and len(nomi_citta) == len(area):
    ...
    print("Non va bene, non hanno la stessa lunghezza.")
    exit()
