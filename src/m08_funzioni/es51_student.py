...
# ## Obiettivo
# Implementare una funzione ricorsiva che calcola l'n-esimo numero della serie di Fibonacci,
#  dove F(0) = 0, F(1) = 1, e F(n) = F(n-1) + F(n-2) per n > 1.
...
def fibonacci(n) -> int:
    if n == 0:
        ...
        return print("Errore: Fibonacci non può essere negativo!")
    else:
...
def main():
    ...
    if n < 0:
            print("fibonacci non può essere negativo..")
    else:
        fibonaccio = fibonacci(n)
        print(f"il numero di fibonacci è {fibonaccio}")
